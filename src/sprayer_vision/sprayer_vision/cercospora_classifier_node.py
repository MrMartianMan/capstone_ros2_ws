...
class CercosporaClassifierNode(Node):
    """
    Standardized classification node.

    Publishes:
    - /sprayer/disease_detected
    - /sprayer/disease_confidence
    - /sprayer/weed_detected   (always False in this mode)
    - /sprayer/weed_confidence (always 0.0 in this mode)
    """

    def __init__(self):
        super().__init__('cercospora_classifier_node')

        self.declare_parameter('model_path', '/home/project-48/models/yolo/cercospora_best.engine')
        self.declare_parameter('image_topic', '/image_raw')
        self.declare_parameter('annotated_image_topic', '/cercospora/image')
        ...
        self.declare_parameter('positive_class_name', 'Cercospora Present')

        model_path = self.get_parameter('model_path').value
        self.image_topic = self.get_parameter('image_topic').value
        self.annotated_image_topic = self.get_parameter('annotated_image_topic').value
        self.conf_threshold = float(self.get_parameter('conf_threshold').value)
        self.required_consecutive_frames = int(self.get_parameter('required_consecutive_frames').value)
        ...
        self.positive_class_name = str(self.get_parameter('positive_class_name').value)

        self.model_path = model_path
        self.model_name = os.path.basename(model_path)

        self.get_logger().info(f'Loading classifier model: {model_path}')
        ...
        self.class_name_pub = self.create_publisher(String, '/sprayer/cercospora_class_name', 10)
        self.annotated_image_pub = self.create_publisher(Image, self.annotated_image_topic, sensor_qos)
    ...
    def image_callback(self, msg):
        ...
        if self.process_every_n_frames > 1 and (self.frame_count % self.process_every_n_frames) != 0:
            return
        ...
        try:
            ...
            self.get_logger().error(f'Classification inference failed: {e}')
            return
        ...
        if len(results) > 0 and hasattr(results[0], 'probs') and results[0].probs is not None:
            ...
            top_class_name = self.model.names[top_idx]

            if top_class_name == self.positive_class_name and top_conf >= self.conf_threshold:
                positive_found = True
        ...
        disease_stable = self.hit_count >= self.required_consecutive_frames

        self.disease_pub.publish(Bool(data=disease_stable))
        self.disease_conf_pub.publish(Float32(data=top_conf))

        self.weed_pub.publish(Bool(data=False))
        self.weed_conf_pub.publish(Float32(data=0.0))

        self.class_name_pub.publish(String(data=top_class_name))

        if self.publish_annotated_image:
            status_text = f'class={top_class_name} conf={top_conf:.2f}'
            stable_text = f'disease={disease_stable}({self.hit_count}) weed=False'
            latency_text = (
            ...
            cv2.putText(
                annotated_frame, stable_text, (20, 70),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
            )
            cv2.putText(
                annotated_frame, latency_text, (20, 105),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2
            )
            cv2.putText(
                annotated_frame, model_text, (20, 140),
